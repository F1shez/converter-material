import { addLineNumberWithError, glEnumToString } from "./utils";
import { DEFAULT_SHADER_TYPE } from "./constants";
...
export const createProgramFromScripts = (
  ...
  shaderScriptIds.forEach((s, i) => {
    shaders.push(createShaderFromScript(gl, s, gl[DEFAULT_SHADER_TYPE[i]]));
  });
...
export const createProgramFromSources = (
  ...
  shaderSources.forEach((s, i) => {
    shaders.push(loadShader(gl, s, gl[DEFAULT_SHADER_TYPE[i]]));
  });
